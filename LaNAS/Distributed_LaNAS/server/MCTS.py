...
class MCTS:
            ...
            try:
                ...
                for i in range(0, 5):
                    ...
                    if conn.poll(0.5):
                        ...
                        if received:
                            self.JOB_COUNTER -= 1
                            print(client_name, "==> net:", job_str, acc, " total samples:", len(self.samples),
                                  " job counter:", self.JOB_COUNTER)
    ...
    def search(self):
        ...
        while len(self.search_space) > 0:
            ...
            print("populate training data###", "total samples:", len(self.samples), " trained:",
                  len(self.DISPATCHED_JOB), " task queue:", len(self.TASK_QUEUE))
            self.print_tree()

            # training the tree
            self.train_nodes()
            print("finishing training###", "total samples:", len(self.samples), " trained:", len(self.DISPATCHED_JOB),
                  " task queue:", len(self.TASK_QUEUE))
            self.print_tree()

            # clear the data in nodes
            print("reset training data###", "total samples:", len(self.samples), " trained:", len(self.DISPATCHED_JOB),
                  " task queue:", len(self.TASK_QUEUE))
            self.reset_node_data()
            self.print_tree()

            print("populate prediction data###", "total samples:", len(self.samples), " trained:",
                  len(self.DISPATCHED_JOB), " task queue:", len(self.TASK_QUEUE))
            self.populate_prediction_data()
            # self.print_tree()

            print("predict###", "total samples:", len(self.samples), " trained:", len(self.DISPATCHED_JOB),
                  " task queue:", len(self.TASK_QUEUE))
            self.predict_nodes()
