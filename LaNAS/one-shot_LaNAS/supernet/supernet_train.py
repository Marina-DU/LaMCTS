...
class Trainer:
    ...
    def __init__(self,
        ...
        print('data=', data, 'batch_size=', batch_size, 'learning_rate=', learning_rate, 'learning_rate_min=',
              ...
              'cutout_length=', cutout_length, 'drop_path_prob=', drop_path_prob, 'seed=', seed, 'grad_clip=',
              grad_clip)
        ...
        self.supernet = Network(supernet_normal, supernet_reduce, layer_type, init_channels, CIFAR_CLASSES, layers,
                                self.criterion, steps=len(supernet_normal))
        self.supernet = self.supernet.cuda()
        self.optimizer = torch.optim.SGD(self.supernet.parameters(), self.learning_rate, momentum=self.momentum,
    ...
    def run(self):
        ...
        for epoch in range(0, self.epochs):
            ...
            torch.save({'epoch': epoch + 1, 'model_state_dict': self.supernet.state_dict(), 'scheduler': self.scheduler,
                       ...
                       os.path.join(self.save_prefix, 'latest_model.pt'))
